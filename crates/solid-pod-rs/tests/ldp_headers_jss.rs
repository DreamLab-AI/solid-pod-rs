...
fn not_found_headers_container_includes_accept_post() {
    ...
    assert!(
        joined.contains("Accept-Post"),
        "missing Accept-Post: {joined}"
    );
    assert!(
...
fn not_found_headers_include_allow_without_delete() {
    ...
    assert!(
        ...
        "Allow must not advertise DELETE on 404: {allow}"
    );
}
...
fn vary_conneg_on_includes_accept() {
    ...
    assert!(
        ...
        "expected Authorization in Vary: {v}"
    );
}
