...
| `/.well-known/nostr.json?name=<local>` (JSS v0.0.190 Phase 1) | (JSS pod-resident NIP-05) | builder primitives in `src/interop.rs::verify_nip05`, `nip05_document` → `Nip05Document` + server route `handle_well_known_nip05` (feature `nip05-endpoint`, default-off, backed by `profile/card` `nostr:pubkey` via `webid::extract_nostr_pubkey`) | ✅ Phase 1 port (alpha.11) | 128, 197 |
| `POST /.pods {provisionKeys: true}` + `--provision-keys` CLI (JSS v0.0.190 Phase 1, issue #437) | (JSS single-user + multi-user pod-keygen surface) | `crates/solid-pod-rs-idp/src/key_provisioning.rs::provision_pod_keys` → `KeyProvisioningOutcome` + `ProvisionPlan::provision_keys` field (cfg-gated) (feature `provision-keys`, default-off) | ✅ Phase 1 port (alpha.11) | 196 |
| `GET /api/exports/all` (JSON-LD time-chain bundle, JSS v0.0.190 Phase 1) | (JSS pod-data export) | `src/export.rs::export_pod_jsonld` → `PodExportBundle` (feature `export-jsonld`, default-off); excludes `/private/*` by default, opt-in via `ExportOptions::include_private` | ✅ Phase 1 port (alpha.11; library surface) | 198 |
